...
Согласно архитектуре, в каждый дискретный момент времени $t$ выполняется преобразование следующее преобразование:

$$\begin{equation}
    \Gamma : S_t \to  y_t,
\end{equation}$$
где
- ${S_t \in \mathbb{R}^{m \times w \times h \times c}}$ - входной набор (clip) из $m$-кадров $I$ с разрешением ${w \times h}$ и количеством каналов $c$,
- ${y_t \in \mathbb{R}^n}$ - класс объекта из общего количества $n$.

Это преобразование осуществляется в следующей последовательности:
### 1. Выделение визуальных фичей (преобразование $F$)

Реализуется с помощью CNN-модели в качестве backbone.

Сначала к каждому кадру ${S_t^j \in S_t}$ применяется преобразование $G$, выделяющее $k$ независимых визуальных фичей.

$$\begin{equation}
    G : S_t^j \to f_t^j,
\end{equation}$$
где ${S_t^j \in \mathbb{R}^{w \times h \times c}}$; ${f_t^j \in \mathbb{R}^{k}}$.
