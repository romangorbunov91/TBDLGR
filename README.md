...
Обучение выполнялось на сбалансированной выборке размера 2863.

Оценка качества работы модели выполнялось после каждой эпохи на сбалансированной валидационной выборке размера 319. Веса модели сохранялись только когда mean accuracy на валидации превосходило максимальное значение с предыдущих эпох обучения.
...
Гиперпараметры задаются в файле [config.json](src/hyperparameters/Bukva/train.json "hyperparameters").
...
Авторы обучали модель на [датасете из кадров](https://drive.google.com/drive/folders/1fNGxhSeYJESwz8WrbrJf83ssl53AIIUm?usp=drive_link "G-Drive"), сформированном из Bukva-trimmed через [framer.py](src/utils/framer.py). 40 кадров на каждое видео.
