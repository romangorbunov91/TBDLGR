...
В основу проекта положена архитектура из статьи
...
В первоисточнике эта архитектура разрабатывалась для распознавания динамических жестов в задачах управления киберфизическими устройствами, в том числе управление по средствам информации с датчиков разных типов одновременно.

В данной архитектуре  выполняется преобразование входного набора (clip) из $m$-кадров $I$ с разрешением ${w \times h}$ и количеством каналов $c$ в распределение вероятностей класса из общего количества $n$-классов:
...
Затем результаты ${f_t^j \in \mathbb{R}^{k}}$ конкатинируются в набор ${f_t \in \mathbb{R}^{m \times k}}$:

$$\begin{equation}
  f_t = f_t^0 \oplus f_t^1 \oplus ... \oplus f_t^m.
\end{equation}$$

Полное преобразование обозначено буквой ${\textbf{F}}$:
...
Энкодинг визуальных фичей в "temporal" фичи реализуется алгоритмом attention.
...
На последнем этапе посредством полносвязной сети формируется распределение вероятностей классов
...
Преобразование видео-данных в наборы кадров выполнено скриптом [framer.py](src/utils/framer.py). Функция `extract_frames` этого скрипт реализует следующую последовательность операций:
1. Видеофайл раскладывается на $r_1$-кадр исходного размера ${w_1 \times h_1}$.
2. Кадры с вертикальной ориентаций 16:9 обрезаются на 20% сверху и 20% снизу, затем приводятся к размеру ${w_2 \times h_2 = 360 \times 360}$. При соотношении сторон отличном от 16:9 размер выбирается по следующим правилам:
- горизонтальный кадр ${\left( w_1 > h_1 \right) \rightarrow w_2 \times h_2 = 360 \times 270}$;
- вертикальный кадр ${\left( w_1 < h_1 \right) \rightarrow w_2 \times h_2 = 270 \times 360}$.
...
Непосредственно на входе архитектуры кадры приводятся к размеру ${224 \times 224}$.
...
## Обучение на датасете [Bukva&copy;](https://github.com/ai-forever/bukva?tab=readme-ov-file#bukva-russian-sign-language-alphabet-dataset)

Тренировочный набор данных размера 2863, достаточно сбалансирован.

В ходе обучения оценка качества работы модели выполнялось после каждой эпохи на достаточно сбалансированной валидационной выборке размера 319. Веса модели сохранялись только когда mean accuracy на валидации превосходила максимальное значение с предыдущих эпох обучения (политика "best").
...
Обучение выполнено на NVIDIA GeForce RTX 3060 (12 GB видеопамяти). Метод оптимизации AdamW (weight_decay=1e-4, base_lr=1e-4), 100 эпох, batch_size=8.
...
Гиперпараметры задаются в файле [config.json](src/hyperparameters/Bukva/config.json "hyperparameters").
...
- **checkpoints-->save_policy** - "all" или "best". Во время обучения в случае "best" веса модели сохраняются на диск только при условии повышения accuracy на валидации, причем предыдущие веса перезатираются.
...
В результате получились ~4000 HD видеозаписей демонстрации жестов разными людьми (>100 видеозаписей на каждый жест).
